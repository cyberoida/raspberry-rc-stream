...
class _Telemetry {
  ...
  parseReply(datastring) {
    ...
    if(splitted[1].startsWith("+CSQ:")) {
      signal = splitted[1].split(" ")[1].split(",")
      signalPer=signal[0]*3+3
      retText = "Signal: " + signalPer + "%"
      return retText
    } elseif(splitted[1].startsWith("+COPS:")) {
      carrier = splitted[1].split(" ")[1].split(",")
      retText = "Carrier: " + carrier[2]
      return retText
    }
  }
  ...
  new_client(socket) {
    ...
    port.on('data', function (data) {
      sockReply=parseReply(data.toString())
