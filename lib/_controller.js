...
*/
  emergency() {
    ...
    if(self.current_speed < 1500) { //fahre geradeaus
      self.pwm.setPulseLength(1, 2000); //voll bremsen
    }
    self.pwm.setPulseLength(1, 1500); //leerlauf
    self.current_speed = 1500;
    clearTimeout(self.emergencyTimeout);
  }
  ...
  new_client(socket) {
    ...
    socket.on("message", function (data) {
      ...
      this.emergencyTimeout = setTimeout(self.emergency, 300);
    });
