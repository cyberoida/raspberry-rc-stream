...
class _Controller {
  ...
  constructor(server) {
    ...
    console.log("Controlling ready to go. setting up telemetry");
  }
  ...
  broadcastTelemetry(local_wss) {
    local_wss.clients.forEach(function (socket) {
  ...
  new_client(socket) {
    ...
    this.telemetry = setInterval(function(){ 
      this.broadcastTelemetry(this.wss), 1000); 
